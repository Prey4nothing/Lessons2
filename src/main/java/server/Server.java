...
import server.authorization.UserData;
...
public class Server {
    ...
    public synchronized void broadcastMessage(ClientHandler nickName, String receiver, String message) {
        for(ClientHandler handler: connectedUsers) {
            if(handler.getNickName().equals(receiver)){
                handler.sendMessage("W from:" + nickName.getNickName() + " " + message);
                break;
            }
        }
    }
